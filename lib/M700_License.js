...
async function checkLicenseFile(context) {
    ...
    common.log('[E700 - E799] checkLicenseFile');

    let data;
    try {
        data = await common.downloadFile(context.githubUrl, '/LICENSE');
    } catch (e) {
        if (e.status == 404) {
            context.errors.push('[E700] File LICENSE not found. Please add.');
        } else {
            context.errors.push(`[E702] Cannot read LICENSE file. Please check. (Status ${e.status})`);
        }
    }
    if (data) {
        ...
        try {
            ...
            //common.debug(`[DEBUG] ${JSON.stringify(npmJson)}`);
            ...
            common.debug(`Error executing "npm view" - ${e}`);
        }
        ...
        if (context.packageJson.license === 'MIT') {
            ...
            if (licenseYear > year) {
                ...
                if (m) {
                    context.errors.push(
                        `[E702] Future year (${licenseYear}) found in LICENSE. Please use "Copyright (c) ${m[1]}-${year} ${getAuthor(context.packageJson.author)}" at the start of LICENSE`,
                    );
                } else {
                    context.errors.push(
                        `[E702]Future year (${licenseYear}) found in LICENSE. Please use "Copyright (c) ${year} ${getAuthor(context.packageJson.author)}" at the start of LICENSE`,
                    );
                }
            } else if (!valid) {
                const m = text.match(/(\d\d\d\d)-\d\d\d\d/);
                if (m) {
                    context.errors.push(
                        `[E701] No actual year found in LICENSE. Please add "Copyright (c) ${m[1]}-${year} ${getAuthor(context.packageJson.author)}" at the start of LICENSE`,
                    );
                } else {
                    context.errors.push(
...
// [700] File LICENSE not found. Please add.');
// [701] No actual year found in LICENSE. Please add "Copyright (c) ${m[1]}-${year} ${getAuthor(context.packageJson.author)}" at the start of LICENSE
// [701] No actual year found in LICENSE. Please add "Copyright (c) ${year} ${getAuthor(context.packageJson.author)}" at the start of LICENSE
// [702] Cannot read LICENSE file. Please check.');
