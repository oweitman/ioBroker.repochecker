...
async function getIOPackageJson(context) {
    common.debug('[init] getting io-package.json');
...
const recommendedAdminVersion = '7.4.10';
let requiredAdminVersion = '6.0.0'; // set to 6.17.14 if jsonConfig is used
...
async function checkIOPackageJson(context) {
    common.log('[E100 - E249] checking io-package.json');
    ...
    if (context.ioPackageJson.common.globalDependencies) {
        if (!(context.ioPackageJson.common.globalDependencies instanceof Array)) {
            ...
            if (adminDependency) {
                ...
                if (!requireAdmin) {
                    context.errors.push(
                        `[S191] admin dependency (${adminDependency['admin']}) at io-package.json is not required. Feel free to remove.`,
                    );
                }
            } else {
                if (requireAdmin) {
        ...
        if (requireAdmin) {
            ...
            if (requiredAdminVersion !== recommendedAdminVersion) {
                context.errors.push(
                    `[E190] admin dependency missing. admin ${requiredAdminVersion} is required as minimum, ${recommendedAdminVersion} is recommended. Please add to dependencies at io-package.json.`,
                );
            } else {
                context.errors.push(
                    `[E190] admin dependency missing. admin ${requiredAdminVersion} is required as minimum. Please add to dependencies at io-package.json.`,
                );
            }
        }
    }
    ...
    if (context.ioPackageJson.common.extIcon) {
        ...
        try {
            if (common.isLocal()) {
                ...
                icon = await common.downloadFile(null, url, true);
            } else {
                icon = await common.downloadFile(context.ioPackageJson.common.extIcon, null, true);
            }
