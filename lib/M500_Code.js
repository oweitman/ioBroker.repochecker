...
async function checkCode(context) {
    common.log('[E500 - E599] checkCode');
    ...
    if (common.isLocal()) {
        ...
        common.debug(`${context.branch}.zip ${data.length} bytes`);
    ...
    if (
        ...
        common.debug('REACT detected at package devDependencies');
        usesReact = true;
    }
    if (
        ...
        common.debug('REACT detected at package dependencies');
        usesReact = true;
    }
    if (context['/src-admin/package.json']) {
        //common.debug('"src-admin/package.json" exists');
        ...
        if (packageJson.devDependencies && packageJson.devDependencies['@iobroker/adapter-react-v5']) {
            common.debug('REACT detected at src-admin/package devDependencies');
            usesReact = true;
        }
        if (packageJson.dependencies && packageJson.dependencies['@iobroker/adapter-react-v5']) {
            common.debug('REACT detected at src-admin/package dependencies');
            usesReact = true;
        }
    }
    if (context['/src-widgets/package.json']) {
        //common.debug('"src-widgets/package.json" exists');
        ...
        if (
            ...
            common.debug('REACT detected at src-widgets/package devDependencies');
            usesReact = true;
        }
        if (
            ...
            common.debug('REACT detected at src-widgets/package dependencies');
            usesReact = true;
        }
    }
    if (context['/src/package.json']) {
        //common.debug('"src/package.json" exists');
        const packageJson = JSON.parse(context['/src/package.json']);
        if (packageJson.devDependencies && packageJson.devDependencies['@iobroker/adapter-react-v5']) {
            // common.debug('REACT detected');
            usesReact = true;
        }
        if (packageJson.dependencies && packageJson.dependencies['@iobroker/adapter-react-v5']) {
            // common.debug('REACT detected');
                ...
                context.cfg.allowedLanguages.forEach(lang => {
                    if (context[`/admin/i18n/${lang}/translations.json`]) {
                        ...
                        context.warnings.push(
                            `[W510] "/admin/i18n/${lang}/translations.json" or "admin/i18n/${lang}.json" not found, but admin support is declared. Please add.`,
                        );
                    }
                });
