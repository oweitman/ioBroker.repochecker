...
async function getPackageJson(context) {
    common.debug('[init] getzting package.json');
...
async function checkPackageJson(context) {
    ...
    if (context.packageJson.publishConfig) {
        context.warnings.push(
            '[S052] "publishConfig" attribute at package.json is not required in modest cases. Please check and eventually remove.',
        );
        if (
