...
const specialAdapters = [
    /*'admin', 'backitup', 'javascript', 'js-controller'*/
...
let requiredNodeVersion = '22'; // This is the minimum node version that must be required at engines clause
const minimumNodeVersion = '22'; // This is the minimum node version that must be required by all adapters at engines clause
...
function getNpmUpdateRule(context, packageName) {
    ...
    const defaultMinimumAgeDays = Number.isFinite(rules.defaultMinimumAgeDays) ? rules.defaultMinimumAgeDays : 180;
...
async function checkNpmDependencyUpdateSuggestions(context) {
    ...
    for (const [sourceType, sourceDeps] of dependencySources) {
        for (const [declaredName, declaredVersion] of Object.entries(sourceDeps)) {
            ...
            if (blackListedDependencies.includes(parsed.packageName)) {
                common.debug(`    ${parsed.packageName} is blacklisted, ignored`);
                continue;
            }
        ...
        await Promise.all(
            batch.map(async checkItem => {
                ...
                try {
                    ...
                    const response = await context.axios(`https://registry.npmjs.org/${encodeURIComponent(packageName)}`);
                    ...
                    const ageThreshold = Math.floor(packageAgeDays / 50) * 50;
                    const ageInfo =
                        ageThreshold >= 50
                            ? ` The newer version has been published for more than ${ageThreshold} days.`
                            : '';
                    context.warnings.push(
                        `[${issueCode}] A newer version (${latestVersion}) for package "${packageName}" (${sourceType}) exists.${ageInfo} Please evaluate if upgrading from ${packageVersion} to ${latestVersion} is possible.`,
                    );
                } catch (e) {
                    common.debug(
...
async function checkPackageJson(context) {
    ...
    if (context.packageJson.scripts && typeof context.packageJson.scripts === 'object') {
 
        const lifecycleScripts = ['preinstall'];
        const forbiddenScripts = ['preprepare', 'prepare', 'postprepare', 'install', 'postinstall'];
        const discouragedScripts = ['prepublish', 'prepublishOnly', 'publish', 'postpublish'];

        lifecycleScripts
            .filter(scriptName => Object.prototype.hasOwnProperty.call(context.packageJson.scripts, scriptName))
            .forEach(scriptName => {
                context.warnings.push(
                    `[E0092] "${scriptName}" found in package.json. Usage of npm install lifecycle scripts should be avoided.`,
                );
            });
        ...
        forbiddenScripts
            ...
            .forEach(scriptName => {
                context.errors.push(
                    `[E0094] "${scriptName}" script is not allowed at ioBroker adapters and needs to be removed.`,
                );
            });

        // react based adapters legitimately use 'publish' and 'prepublish' scripts to build the UI,
        // so these two scripts must not be flagged if the adapter uses react
        const reactAllowedScripts = context.usesReact ? ['prepublish', 'prepublishOnly'] : [];

        discouragedScripts
            .filter(scriptName => !reactAllowedScripts.includes(scriptName))
            .filter(scriptName => Object.prototype.hasOwnProperty.call(context.packageJson.scripts, scriptName))
            .forEach(scriptName => {
                context.warnings.push(
                    `[W0095] "${scriptName}" script is not needed in most cases when using the standard ioBroker workflow test-and-release. Usage should be reviewed.`,
                );
            });
    }
            ...
            if (!context.packageJson.engines.node) {
                ...
                if (!minimumNodeJsSupported) {
                    ...
                    if (!compareVersions.compare(minimumNodeJsSupported, requiredNodeVersion, '>=')) {
                        context.errors.push(
                            `[E0029] Node.js ${requiredNodeVersion} is required as minimum, node.js ${recommendedNodeVersion} is recommended. The current engines.node clause "${nodeVal}" allows Node.js ${minimumNodeJsSupported} as minimum. Please update it to require at least ">=${requiredNodeVersion}" at package.json.`,
                        );
                    } else if (!compareVersions.compare(minimumNodeJsSupported, recommendedNodeVersion, '>=')) {
                        context.warnings.push(
                            `[W0028] Minimum node.js version ${recommendedNodeVersion} recommended. The current engines.node clause "${nodeVal}" allows Node.js ${minimumNodeJsSupported} as minimum. Please update it to require at least ">=${recommendedNodeVersion}" at package.json.`,
                        );
                    } else {
                        context.checks.push(
    ...
    if (minimumNodeJsSupported === null && !context.cfg.onlyWWW) {
        minimumNodeJsSupported = '20.0.0'; // ensure valid semver
    }
    ...
    if (context.packageJson.devDependencies && context.packageJson.devDependencies['@iobroker/testing']) {
        ...
        try {
            if (compareVersions.compare(testingVersion, '5.1.1', '>=')) {
                ...
                // Stored into context.redundantTestingPackages so later modules
                // (e.g. M5000_Code W5042) can reuse it.
                ...
                context.redundantTestingPackages = redundantPackages;
    ...
    if (hasEslintConfig) {
        if (!fileExistsInList(context, '/eslint.config.mjs')) {
            ...
            try {
                ...
                const hasImport = /import\s+.*\s+from\s+['"]@iobroker\/eslint-config['"]\s*;?/.test(
                    String(eslintConfigContent || ''),
                );
                if (!hasImport) {
                    context.errors.push(
                        '[E0077] "eslint.config.mjs" should use installed "@iobroker/eslint-config" via `import ... from \'@iobroker/eslint-config\';`.',
                    );
                } else {
...
// [W0092] "preinstall" script found in package.json. Usage of npm install lifecycle scripts should be avoided.
// [E0093] Script(s) ${forbiddenInstallScripts.map(scriptName => `"${scriptName}"`).join(', ')} found in package.json. Remove npm install lifecycle scripts except "preinstall".
