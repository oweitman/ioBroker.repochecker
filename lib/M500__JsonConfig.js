    ...
    if (responsiveWarningLimit) {
        ...
        if (responsiveWarningLimit === 0) {
            context.warnings.push(
                `[W510] responsive check: maximum issues reached, please fix reported ones and recheck`,
            );
        }
    }
}
...
function chk_responsive(context, path, jsonConfig, component, opts) {
    ...
    if (!opts.sizesOptional) {
        ...
        if (missing.length) {
            handleResponsiveError(
                context,
                `[E507] missing size attributes [${missing.join(',')}] for ${component} at ${path}`,
            );
        }
    }
    ...
    for (const size of sizes) {
        if (jsonConfig[size] !== undefined && (jsonConfig[size] > 12 || jsonConfig[size] <= 0)) {
            handleResponsiveError(
                context,
                `[E511] invalid size attributes "${size}":"${jsonConfig[size]}" for ${component} at ${path}. Value must be 1 to 12.`,
            );
        }
    }
    ...
    if (!opts.sizesOptional && jsonConfig.xs && jsonConfig.xs != 12) {
        handleResponsiveWarning(
            context,
            `[W508] attribute "xs" for ${component} should specify a value of "12" at ${path}`,
        );
    }
    ...
    if (jsonConfig.sm && jsonConfig.sm > ii) {
        handleResponsiveError(
            context,
            `[E509] "sm" value (${jsonConfig.sm}) for ${component} greater then value for smaller displays`,
        );
    } else {
        ii = jsonConfig.sm;
    }
    if (jsonConfig.md && jsonConfig.md > ii) {
        handleResponsiveError(
            context,
            `[E509] "md" value (${jsonConfig.md}) for ${component} greater then value for smaller displays`,
        );
    } else {
        ii = jsonConfig.md;
    }
    if (jsonConfig.lg && jsonConfig.lg > ii) {
        handleResponsiveError(
            context,
            `[E509] "lg" value (${jsonConfig.lg}) for ${component} greater then value for smaller displays`,
        );
    } else {
        ii = jsonConfig.lg;
    }
    if (jsonConfig.xl && jsonConfig.xl > ii) {
        handleResponsiveError(
            context,
            `[E509] "xl" value (${jsonConfig.xl}) for ${component} greater then value for smaller displays`,
        );
    }
}
...
function checkComponent(context, path, jsonConfig) {
    ...
    if (!type) {
        context.errors.push(`[E502] no type specified at ${path}`);
    }
    common.debug(`  type ${type}`);

    if (validComponents[type] === undefined) {
        context.errors.push(`[E504] unexpected component "${type}" detected at ${path}`);
...
function checkPanel(context, path, jsonConfig, isRoot) {
    ...
    //common.log('on PANEL entry');
    //common.log(jsonConfig);
    ...
    for (const key in jsonConfig) {
        ...
        if (!validAttributes.includes(key)) {
            context.errors.push(`[E503] unexpected type "${key}" detected at ${path}`);
        }

        if (key === 'type') {
            if (jsonConfig[key] !== 'panel') {
                common.error(`INTERNAL ERROR: unexpected type ${jsonConfig[key]} encountered while scanning "panel"`);
                return;
            }
        }
...
function checkTabs(context, path, jsonConfig) {
    ...
    for (const key in jsonConfig) {
        ...
        if (!type) {
            context.errors.push(`[E502] no type specified at ${path}; expected "panel"`);
        }
        if (type !== 'panel') {
            context.errors.push(`[E503] unexpected type "${key}" detected at ${path}; expected "panel"`);
        } else {
...
function checkJsonConfig(context, path, jsonConfig) {
    ...
    let baseType = 'panel';
    if (!jsonConfig['type']) {
        common.log(`*ERROR* base type (panel / tabs) not specified`);
    } else {
        baseType = jsonConfig['type'];
        common.debug(`  base type ${baseType}`);
        if (!validTypes.includes(baseType)) {
            context.errors.push(`[E500] unexpected base type "!${baseType}" detected at ${path}`);
        }
    }
    ...
    for (const key in jsonConfig) {
        ...
        if (!validAttributes.includes(key)) {
            context.errors.push(`[E501] unexpected attribute "${key}" detected at ${path}`);
        }
        if (key === 'items') {
            if (baseType == 'panel') {
...
// [500] unexpected base type "!${baseType}" detected at ${path}
// [501] unexpected attribute "${key}" detected at ${path}
// [502] no type specified at ${path}
// [503] unexpected type "${key}" detected at ${path}
// [504] unexpected component "${type}" detected at ${path}
// [505]
// [506]
// [507] responsive check: missing size attributes [${missing.join(',')}] at ${path}
// [508] responsive check: attribute "xs" should specify a value of "12" at ${path}
// [509] responsive check: "sm" value (${jsonConfig.sm}) greater then value for smaller displays
// [510] responsive check: maximum issues reached, please fix reported ones and recheck
// [511] invalid size attributes "${size}":"${jsonConfig[size]}" for ${component} at ${path}. Value must be 1 to 12.
